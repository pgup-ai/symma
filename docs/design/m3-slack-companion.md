...
3. On the machine where their agents are already logged in, the member runs the
   ...
   `pair` sends the code to `SYMMA_COMPANION_GATEWAY`, defaulting to
   `https://symma.dev`. A default is the point: without one the command needs a
   second field, which is the flow this section exists to remove.
