...
with `groups:read` to tell a public channel from a private one before fetching a
link into it, `chat:write` with `im:write` to answer there, and `reactions:write`
